...
impl Model for Env {
    fn add_project(&mut self, item: NewProject) -> ProjectID {
        ...
        diesel::insert_into(projects::table)
            ...
            .execute(&db_con)
            .unwrap();
        last_id!(projects::project_id, &db_con)
    }
    ...
    fn add_worker(&mut self, item: NewWorker) -> WorkerID {
        ...
        diesel::insert_into(workers::table)
            ...
            .execute(&db_con)
            .unwrap();
        last_id!(workers::worker_id, &db_con)
    }
    ...
    fn get_projects(&self) -> Vec<Project> {
        let db_con = db::open();
        projects::table.load::<Project>(&db_con)
            .unwrap()
    }
    
    fn get_workers(&self) -> Vec<Worker> {
        let db_con = db::open();
        workers::table.load::<Worker>(&db_con)
            .unwrap()
    }
    
    fn get_tasks(&self) -> Vec<Task> {
        let db_con = db::open();
        tasks::table.load::<Task>(&db_con)
            .unwrap()
    }
    ...
    fn get_project_tasks(&self, project_id: ProjectID) -> Vec<Task> {
        let db_con = db::open();
        tasks::table.filter(tasks::project_id.eq(project_id))
    ...
    fn get_worker_tasks(&self, worker_id: WorkerID) -> Vec<Task> {
        ...
        rows.drain(..).map(row_to_task).collect()
    }
